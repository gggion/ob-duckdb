...
- [[#examples-from-the-docs][Examples from the docs]]
  - [[#readme-example-1-basic-query][README Example 1: Basic Query]]
  - [[#readme-example-2-database-file][README Example 2: Database File]]
  - [[#readme-example-3-async-execution][README Example 3: Async Execution]]
  - [[#readme-example-4-session-persistence][README Example 4: Session Persistence]]
- [[#cleanup][Cleanup]]
:END:
...
#+begin_src emacs-lisp
(setq org-babel-duckdb-prompt-char "🦆>")
#+end_src
...
:PROPERTIES:
:CUSTOM_ID: examples-from-the-docs
:END:
** README Example 1: Basic Query
:PROPERTIES:
:CUSTOM_ID: readme-example-1-basic-query
:END:

#+NAME: doc-example-basic
#+begin_src duckdb
SELECT * FROM generate_series(1, 5) AS s(num);
#+end_src

** README Example 2: Database File
:PROPERTIES:
:CUSTOM_ID: readme-example-2-database-file
:END:

#+NAME: doc-example-db-file
#+begin_src duckdb :db /tmp/readme-example.duckdb
...
** README Example 3: Async Execution
:PROPERTIES:
:CUSTOM_ID: readme-example-3-async-execution
:END:

#+NAME: doc-example-async
#+begin_src duckdb :async yes :session readme-async :var parquet_url='https://github.com/apache/parquet-mr/raw/master/parquet-hadoop/src/test/resources/test-file-with-no-column-indexes-1.parquet'
SELECT * FROM read_parquet('parquet_url') WHERE id < 10;
#+end_src

** README Example 4: Session Persistence
:PROPERTIES:
:CUSTOM_ID: readme-example-4-session-persistence
:END:

#+NAME: doc-example-session-1
#+begin_src duckdb :session readme-session
CREATE TEMPORARY TABLE temp_data AS
  SELECT * FROM generate_series(1, 10) AS t(num);
SELECT 'Table created' AS status;
#+end_src

#+NAME: doc-example-session-2
#+begin_src duckdb :session readme-session
SELECT SUM(num) AS total, AVG(num) AS average FROM temp_data;
#+end_src
